...
async function inspect(page, label, terminal=false) {
  const metrics = await page.evaluate(() => {
    ...
    return {w:innerWidth,h:innerHeight,sw:d.scrollWidth,sh:d.scrollHeight,clip,text:document.body.innerText};
  });
...
for(const type of (process.env.QA_ENGINE==='chromium' ? [chromium]:[chromium,webkit])){
 ...
 for(const [width,height] of [[390,844],[375,812],[393,852],[430,932],[768,1024],[1440,1000]]){
  ...
  for(let i=0;i<7;i++)await cards.nth(i).getByRole('button',{name:i<5?'Écarter':i===5?'Incertain':'Retenir',exact:true}).click();
